...
import { updateFarmerProfile, clearAuth } from '../services/api';
...
export default function FarmerManageProfile() {
  ...
  const [profile, setProfile] = useState({
    ...
    username: 'johndoe_agri',
    email: 'john.doe@example.agri.gov',
    phone: '550-012-3456',
    sex: 'M',
    password: '',
    address: '482 Green Valley Road, Fairfield',
    farmer_card: 'F-8892-XXXX',
    farm_area: '120',
    farm_location: 'Central Plains'
  });

  const [profilePic, setProfilePic] = useState('https://lh3.googleusercontent.com/aida-public/AB6AXuBA8wKl0ndUbmqnFd-rdEjLVGJV8k1yesoa6UP7pAQ70-md3gf4FoB3jkC36bH18jFHSwGqV-oKsnmjW8_WSt2HNJkF0RtsJMAYyBvxUsf7Kqu6XwA1g53Rh36PkqtRTUI7iXtZJLjYExg9QbM8GgYFyyXR27Hbb4CPzjcb5sBZqJTltWQMgdBLqKdmoOdOuzkOjjsbkN8aCFSjYebLyoKdOAMJOyzvF0pYgrvRwbuyVtYAy6p-dErnX4hFT0_v700lzZjVddhr1gM');
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState('');

  const farmerInitials = profile.full_name?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) || 'FD';
  ...
  // BACKEND INTEGRATION:
  // This function is triggered when the farmer clicks "Save Changes".
  // The backend developer should capture the "profile" state and any
  // image data to send via a PUT/POST request to their API endpoint.
  const handleSaveProfile = async () => {
    setLoading(true);
    setMessage('');
    try {
      await updateFarmerProfile({ ...profile, profile_picture: profilePic });
      ...
      <div className="flex min-h-screen relative">
        ...
        <aside className="hidden md:flex w-64 bg-surface border-r border-outline-variant/30 flex-col fixed h-full z-50">
          ...
          <nav className="flex-1 px-4 space-y-2 mt-4">
            ...
            <a href="/farmer/profile" className="flex items-center gap-3 px-4 py-3 rounded-lg bg-primary text-on-primary shadow-md shadow-primary/20 cursor-pointer">
              <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1" }}>person</span>
              <span className="font-medium">Profile</span>
            </a>
            <a href="/farmer/products" className="flex items-center gap-3 px-4 py-3 rounded-lg text-on-surface-variant hover:bg-surface-container-high transition-colors cursor-pointer">
              <span className="material-symbols-outlined">inventory_2</span>
              <span className="font-medium">My Products</span>
            </a>
            {/* Crops, Inventory, Settings removed as requested */}
          </nav>
