...
# 4. جلب قائمة الحسابات المفعلة والمحظورة (Validated)
@api_view(['GET'])
@permission_classes([IsAdminUser])
def get_validated_users(request):
    ...
    serializer = UserSerializer(validated_users, many=True)
    return Response(serializer.data, status=200)

# 5. قبول أو رفض مستخدم جديد
@api_view(['POST'])
@permission_classes([IsAdminUser])
...
# 6. حظر أو إلغاء حظر مستخدم مفعل (Toggle Block)
@api_view(['POST'])
@permission_classes([IsAdminUser])
def toggle_user_block(request, user_id):
    try:
        ...
        if user.status == 'active':
            user.status = 'blocked'
            user.is_active = False
            subject = "تنبيه: تم حظر حسابك في AgriGov ⚠️"
            message = f"مرحباً {user.full_name}، نأسف لإعلامك أنه تم حظر حسابك مؤقتاً من طرف الإدارة."
            res_msg = f"تم حظر المستخدم {user.full_name} بنجاح."
        elif user.status == 'blocked':
            user.status = 'active'
            user.is_active = True
            subject = "تم رفع الحظر عن حسابك في AgriGov ✅"
            message = f"مرحباً {user.full_name}، تم تفعيل حسابك مجدداً. يمكنك الآن الدخول للمنصة."
            res_msg = f"تم إلغاء الحظر عن المستخدم {user.full_name}."
        else:
            return Response({'error': 'لا يمكن حظر مستخدم في حالة الانتظار أو الرفض.'}, status=400)
...
# 7. إحصائيات لوحة التحكم (Dashboard Stats)
@api_view(['GET'])
@permission_classes([IsAdminUser])
